...
public class Maze {
	 ...
	 */
	public List<Coordinate> dfs(Coordinate s, Coordinate g) {
		
		// Initialize everything
		MazeNode start = new MazeNode(s.getRow(), s.getCol());
		MazeNode goal = new MazeNode(g.getRow(), g.getCol());
	...
	private LinkedList<Coordinate> constructPath(MazeNode start, MazeNode goal, HashMap<MazeNode, MazeNode> parentMap) 
	{
		LinkedList<Coordinate> path = new LinkedList<Coordinate>();
		MazeNode curr = goal;
		while (curr != start) {
			path.addFirst(new Coordinate(curr.getRow(), curr.getColumn()));
			curr = parentMap.get(curr);
		}
		path.addFirst(new Coordinate(start.getRow(), start.getColumn()));
		return path;
	}
